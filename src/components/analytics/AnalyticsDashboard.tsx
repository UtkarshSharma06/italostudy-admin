...
export default function AnalyticsDashboard({ userId: propUserId, isMobile: propIsMobile, hideLayout = false }: AnalyticsDashboardProps) {
    ...
    useEffect(() => {
        const fetchTargetProfile = async () => {
            ...
            // Fetch full profile to get their exam ID
    ...
    useEffect(() => {
        ...
        const fetchAnalytics = async () => {
            // Guard against missing essential IDs
            if (!targetUserId) {
            ...
            if (!effectiveExamId) {
                // For non-self views, if profile is loaded but has no exam, try a common default
                if (!isSelfView && targetProfile) {
                    // Profile loaded but has no exam ID, use active exam as fallback
                    // Don't block - proceed with activeExam
                } else {
                    // Wait briefly for context to provide exam ID
            ...
            if (!isSelfView && !targetProfile) {
                // Wait for target profile to be loaded, then the effect will re-run
                return;
            }
    ...
    }, [targetUserId, effectiveExamId, timeframe, targetProfile, isPremiumViewer, isSelfView, selectedSubject]);
